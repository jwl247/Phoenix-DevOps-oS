...
# No logging.basicConfig here — this is a library. The boot entrypoint
# (sector1/kernel/main_kernel.py) owns root logging config.
...
class GuardianRotator:
    ...
    def __init__(self, min_interval=180, max_interval=600):
        ...
        # Set by copes_runtime.build() — called with the incident dict whenever
        # a honeypot is probed. None = incidents are only logged locally.
    ...
    def honeypot_triggered(self, guardian_id: GuardianID, incident_data: dict):
        ...
        # Upstream escalation (copes_runtime persists it + calls the app sink)
        if self.escalation_sink:
            try:
                self.escalation_sink(incident)
            except Exception as e:  # noqa: BLE001 — never break the reporter
                log.warning(f"escalation_sink raised (ignored): {e}")
