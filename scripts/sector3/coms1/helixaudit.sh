...
# HELIX AUDIT v1.0
# Scans SECTOR3 files and flags anything that lost its helix/quad structure
...
SECTOR3 = Path("/etc/systemd/system/SECTOR3")
...
def scan():
    print("💎 HELIX AUDIT — Scanning SECTOR3...\n")
    
    for f in SECTOR3.rglob("*"):
        if f.is_file():
            if f.suffix == ".py":
                check_python_file(f)
            elif f.suffix == ".json":
