...
# ── Backend Detection ────────────────────────────────────────
detect_backend() {
    # Phoenix/UnitedSys native — clone pool takes priority
    command -v clone_pool &>/dev/null && echo "clone_pool" && return 0
...
translate_cmd() {
    ...
    case "${backend}" in
        ...
        brew)
            case "${verb}" in
                install)   echo "brew install ${pkg}" ;;
                remove)    echo "brew uninstall ${pkg}" ;;
                update)    echo "brew update" ;;
                upgrade)   echo "brew upgrade" ;;
                search)    echo "brew search ${pkg}" ;;
                info)      echo "brew info ${pkg}" ;;
                list)      echo "brew list" ;;
                clean)     echo "brew cleanup" ;;
                *)         echo "UNKNOWN_VERB:${verb}" ; return 1 ;;
            esac ;;
        clone_pool)
            # Phoenix/UnitedSys native — verbs route through clone pool directly
            case "${verb}" in
                install)   echo "clone_pool fetch ${pkg}" ;;
                remove)    echo "clone_pool drop ${pkg}" ;;
                update)    echo "clone_pool sync" ;;
                upgrade)   echo "clone_pool refresh" ;;
                search)    echo "clone_pool query ${pkg}" ;;
                info)      echo "clone_pool inspect ${pkg}" ;;
                list)      echo "clone_pool list" ;;
                clean)     echo "clone_pool purge" ;;
                *)         echo "UNKNOWN_VERB:${verb}" ; return 1 ;;
            esac ;;
        *)
...
Backends supported:
  clone_pool (Phoenix/UnitedSys native — priority)
  apt | dnf | pacman | zypper | apk | xbps | portage | brew
  winget | choco
