...
class HelixCache:
    ...
    def _get_tier_size(self, tier_dict: OrderedDict[str, CacheBlock]) -> int:
        total = 0
...
class HelixMemoryManager:
    def __init__(self, cache: HelixCache, max_virtual_mb=8192):
        self.cache = cache
...
class HelixTranslator:
    def __init__(self, helix_system: 'HelixSystem'):
...
class AgnosticLayer:
    ...
    def accelerate_desktop(self):
        """Primary entry point: safely lighten the host desktop."""
        sysinfo = self.system
        print("⚡ AgnosticLayer: Desktop acceleration pass starting...")

        if sysinfo.os_type == OSType.WINDOWS:
            self._accelerate_windows(sysinfo)
        elif sysinfo.os_type in (OSType.LINUX, OSType.MACOS):
            self._accelerate_unix(sysinfo)
        else:
            print("  Unknown OS — no acceleration performed.")

        print("⚡ AgnosticLayer: Desktop acceleration pass complete.")

    def _accelerate_windows(self, sysinfo: SystemInfo):
        temp_path = str(sysinfo.temp_dir)
        print(f"  [Windows] Clearing TEMP at {temp_path}")
        try:
            subprocess.run(
                ["cmd.exe", "/c", f"del /q/f/s \"{temp_path}\\*\""],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
                shell=False,
            )
        except Exception as e:
            print(f"  [Windows] TEMP cleanup failed: {e}")

        try:
            import psutil
            heavy = ["chrome.exe", "msedge.exe", "teams.exe"]
            for proc in psutil.process_iter(['pid', 'name', 'cpu_percent']):
                name = (proc.info.get('name') or "").lower()
                if name in heavy and proc.info.get('cpu_percent', 0) > 40:
                    try:
                        proc.nice(psutil.BELOW_NORMAL_PRIORITY_CLASS)
                        print(f"  [Windows] Lowered priority: {name} (PID {proc.pid})")
                    except Exception:
                        pass
        except ImportError:
            print("  [Windows] psutil not installed; skipping process tuning.")

    def _accelerate_unix(self, sysinfo: SystemInfo):
        temp_candidates = [sysinfo.temp_dir, Path("/tmp")]
        for t in temp_candidates:
            if t.exists():
                print(f"  [Unix] Clearing temp at {t}")
                try:
                    subprocess.run(
                        ["bash", "-lc", f"rm -rf \"{t}\"/*"],
                        stdout=subprocess.DEVNULL,
                    ...
                    print(f"  [Unix] Temp cleanup failed: {e}")

        if sysinfo.os_type == OSType.LINUX and sysinfo.has_sudo:
            print("  [Linux] Requesting page cache drop (sudo)")
            try:
                subprocess.run(
                    ["sudo", "bash", "-lc", "sync; echo 3 > /proc/sys/vm/drop_caches"],
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                    shell=False,
                )
            except Exception as e:
                print(f"  [Linux] Cache drop failed: {e}")
...
class HelixSync:
    ...
    def _api(self, method: str, path: str, body: dict = None) -> Optional[dict]:
        ...
        if urllib_request is None:
            self._log('error', 'urllib not available for API calls')
            return None
    ...
    def ensure_syncthing(self) -> bool:
        ...
        if platform.system().lower().startswith("windows"):
            self._log('warning', 'Syncthing auto-install disabled on Windows')
...
<configuration version="37">
    ...
    def _on_sync_complete(self):
        ...
        # After successful sync, optionally accelerate desktop
        global _agnostic
        if _agnostic is not None:
            self._log('info', 'Triggering desktop acceleration after sync')
            _agnostic.accelerate_desktop()
...
_helix: Optional[HelixSystem] = None
_translator: Optional[HelixTranslator] = None
_agnostic: Optional[AgnosticLayer] = None
...
def init_helix(l1_mb=512, l2_mb=2048, l3_mb=6000, vram_mb=8096):
    ...
    # FIRST REFLEX: speed up the desktop right after Helix comes online
    _agnostic.accelerate_desktop()
...
def main():
    ...
    print("-" * 70)
    sync = init_sync()  # init only, don't start Syncthing daemon
    sync.print_status()
