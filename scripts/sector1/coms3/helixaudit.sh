...
# HELIX AUDIT v1.0
# Scans SECTOR1 files and flags anything that lost its helix/quad structure
...
SECTOR1 = Path("/etc/systemd/system/SECTOR1")
...
def scan():
    print("💎 HELIX AUDIT — Scanning SECTOR1...\n")
    
    for f in SECTOR1.rglob("*"):
        if f.is_file():
            if f.suffix == ".py":
                check_python_file(f)
            elif f.suffix == ".json":
