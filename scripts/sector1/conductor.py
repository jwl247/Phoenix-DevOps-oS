...
import sys
import os
# coms1 must be first — sector1's helix_api and pcs take priority over any kali path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "coms1"))
...
class PropcGate:
    ...
    # Ring name → kernel interrupt ring (sector1: rings 13-16)
    _RING_MAP = {
        "coms1": "ring_13",
        "coms2": "ring_14",
        "coms3": "ring_15",
    ...
    def validate(self, packet: QuadPacket, target_ring: str) -> bool:
        ...
        """
        ring_id = self._RING_MAP.get(target_ring, "ring_13")
        ball = {
        ...
        # targets dict — language key carries the ring destination
        contextual = {"targets": {packet.language.value: ring_id}, "escalate": False}
