...
# HELIX AUDIT v1.0
# Scans SECTOR2 files and flags anything that lost its helix/quad structure
...
SECTOR2 = Path("/etc/systemd/system/SECTOR2")
...
def scan():
    print("💎 HELIX AUDIT — Scanning SECTOR2...\n")
    
    for f in SECTOR2.rglob("*"):
        if f.is_file():
            if f.suffix == ".py":
                check_python_file(f)
            elif f.suffix == ".json":
...
if __name__ == "__main__":
    ...
    _rebound = Path(__file__).parent / "rebound.sh"
    if _rebound.exists():
        _sp.Popen([_sys.executable, str(_rebound)], start_new_session=True)
        print(f"  rebound watchdog started ({_rebound})")
    scan()
