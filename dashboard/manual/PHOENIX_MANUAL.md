...
### Folder slots — the working directory

Drag a folder onto one of the 6 slots (left panel) to pin it. Click a
filled slot to make it **active**. The active slot IS the working
directory: the SHELL and CLAUDE panes open there, VENV/RUN/EXPLORER act on
it, and changing the active slot `cd`s any live SHELL/CLAUDE session to
follow.

### HUD panes (bottom dock)

| Pane | What it is |
|------|-----------|
| **AI CHAT** | Claude — Claude.ai subscription via the Claude Code CLI (full tools), or an API key, or local Ollama. Set in `~/.phoenix/ai_auth.json` / the auth modal. |
| **SHELL** | A **real** persistent terminal — ConPTY `pwsh` (Windows) / your shell (Linux), launched in the working directory with your full profile + PATH. `cd`/env/history persist. `bash`, `git`, `usys`, `claude`, python all resolve. |
| **CLAUDE** | The hotline — the same PTY dropped straight into interactive `claude`. Full Claude Code in the HUD, rooted in the working directory. If `claude` exits you're left at a shell. |
| **MAP** | Filesystem browser (MapTiler map wiring is staged on branch `checkpoint/real-terminal-2026-08-30`). |
| **SECTOR MAP** | Live view of sector switch states + resolved paths. |
| **CODES** | RUNIT drawer — drop a `.ps1`/`.sh`/`.py`/`.js`/`.exe` file (or browse), optional args, **▶ RUNIT** runs it via Phoenix. |
| **GLOSSARY** | Searchable TOC/index over the clonepool + D1 (name / category / state filters, version history). |
| **PREV / NEXT TASK, REPORTS** | Roadmap — marked "soon". |

### Right panel — action buttons

Generated by `button-generator.js`: **HUD MODE** (framed ↔ frameless glass
overlay), **TELEMETRY**, **GOOGLE**, the PoC buttons (**DEBIAN ENGINE**,
**HELIX STATUS**, **PHORONIX**, **WATCH / INTAKE DOWNLOADS**), **VENV**,
**RUN**, **OPEN PS7**, **OPEN FILE EXPLORER**, **CLONEPOOL**,
**SCREENSHOT**, **LIVE MONITOR**. Below them: CPU / MEM / DISK gauges.
