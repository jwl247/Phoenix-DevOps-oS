...
- [x] Security Gap 1 — every GET route gated + a leftover Cloudflare Access "bypass, everyone" policy (dated 2026-03-27, silently defeating Access this whole time) removed, service-token policy fixed (needed its own `non_identity` decision, not folded into the identity `allow` policy). Deployed and verified live 2026-09-21. Gap 2 (content encryption) still open — see NEXT SESSION + `project_security_gap_plan` memory.
...
## NEXT SESSION
- **POST-REBASE REGRESSION CHECK — start here, before anything else.** 2026-09-21's git divergence rebase merged two independently-evolved histories by hand across `intake.sh`/`worker/index.js`/`CLAUDE.md` — real risk that something real got silently dropped or half-merged even though it looked clean at the time. Jerry's explicit ask: confirm these still actually work, not just that the code compiles/parses:
  - **Global clone-to-workdir** — `usys open <name>.lol` / `clone <name>` from a plain terminal outside the repo (not just the dashboard's embedded shell). Confirm it still resolves via the PowerShell profile hook and actually pulls the right file.
  - **R2 sync** — a real intake → confirm bytes actually land in R2 via `packages-worker`'s integrated route (the one that replaced `phoenix-clonepool-r2` this session) and fetch back byte-identical.
  - **Directory intake** (`intake_directory` / snapshot form) — still produces a correct manifest, still gets real hex/QR/D1 rows per file, not just the top-level summary entry.
  - **Auto-intake / watch-downloads** (`watch-downloads.ps1`, `usys watch`, the `Start-UsysWatcher` path) — confirm files landing in `F:\Phoenix\Downloads` still auto-intake via `phx-import`, and that it survived both the R2 consolidation (new upload route) and the new Cloudflare Access service-token requirement (does the watcher's background job actually have `CF_ACCESS_CLIENT_ID`/`SECRET` in its environment, or only the interactive shell that set them today?).
  - Also worth a quick check: `intake prune` / `rotate_clonepool_tiers()` still runs clean end-to-end after the R2 route change.
...
- **SECURITY Gap 2 (content encryption) — start here, highest priority open item.** Gap 1 (open GET auth + a leftover Cloudflare Access bypass-everyone policy) is FIXED and deployed as of 2026-09-21 — see SESSION LOG. Gap 2 remains: "quadralingual" storage was never a confidentiality mechanism — real code exists (`archive/.../freewheeling.py` `QuadralingualPacket`) but two of its four views store plaintext unmodified; no encryption/obfuscation of file content has ever existed anywhere in Phoenix. Key-management design decided 2026-09-21 (this PC holds the key — it's the always-on new PC, not the Debian PoC; Windows fingerprint code already exists in `sector2/apps/office/lib/fingerprint.js`, reusable as-is). Still to pick: cipher (AES-256-GCM assumed) + scope (everything vs. `sensitive=1` only). Still to build: the actual encrypt/decrypt wiring. Full detail — see memory `project_security_gap_plan.md`, don't re-derive.
