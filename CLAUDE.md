...
- [x] Desktop — mixing board repurposed as service control surface, live at /desktop/
  - 12 channel strips (SSH, WireGuard, Cloudflared, Frank, Conversion, Ollama, Life First, Nextcloud, Prometheus, venv×3)
  - VU meters = active/inactive; ON/OFF/RST/LOG buttons; Master section (All On / All Off)
  - Telemetry panel: CPU %, RAM %, Swap %, Threat Level 1-5 DefCon display (polls every 8s)
  - Threat derived from auth.log + load + zombie count + live security stack buffer level
- [x] Switches — toggles (Bounce Armed/red, Full Audit/amber, Auto-Forge/amber, Guardian/green, Prometheus/green) + dropdowns (Ollama model, lockdown level, threat response, buffer sensitivity) + action buttons, live at /desktop/switches.php
- [x] File Tree — real filesystem browser, drag+drop group assignment, user-defined pinnable groups, forged D1 docs, live at /desktop/filetree.php
- [x] Global Shell — backtick/F12 Quake-style drop-down terminal from anywhere in Desktop; builtins (status/threat/services/frank/ollama/wg/mixer/switches/files/clear); ask <question> → Phoenix AI (Ollama primary → Claude fallback); run <cmd> → api/shell.php exec wrapper with 30s timeout + blocklist + audit log
- [x] Security stack — FileMotionSensor, BufferEscalationSystem, LockdownDefenseSystem, SignalMirrorBouncer (amplificationFactor=1 LOCKED), InstallerGuardian+FileGuardian — all wired with real Linux (lsof/ss/ufw/chattr), D1 forensic logging, Frank escalation — private submodule github.com/jwl247/phoenix-security, sector4/security/ gitignored
- [x] Operator Manual — Desktop section added (14 sections total): Global Shell hotkeys, Mixer channels/threat, Switches table, File Tree groups, deploy command
...
2026-06-17 — Phoenix Desktop control surface LIVE. Mixing board repurposed: 12 service channel strips, VU meters = health, telemetry panel (CPU/RAM/Swap/Threat DefCon 1-5). Switches pane: toggles (Bounce Armed red, Full Audit amber, Auto-Forge amber, Guardian/Prometheus green) + 4 dropdowns + 5 action buttons. File Tree: real filesystem drag+drop with user-defined pinnable groups, D1 forged docs wired in. Global Shell: backtick/F12 Quake drop-down terminal — builtins, 'run <cmd>' via api/shell.php (proc_open, 30s timeout, blocklist, audit), 'ask <question>' → Frank :7347 → Ollama primary → Claude fallback → green shell output. Security stack (5 modules — motion/buffer/lockdown/bouncer/guardian) wired with real Linux primitives, private submodule gitignored. Operator Manual: Desktop section added (now 14 sections). All pushed, deployed to phoenix-ext. Next: pull phi3.5, pre-warm llama3.1, Office pane, rotate Claude API key.
