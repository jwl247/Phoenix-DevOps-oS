...
## NEXT SESSION
- **DEPLOY meds-worker — deadline was Mon 2026-09-14, today is 2026-09-21: check with Jerry whether this shipped already (by hand, outside this checkout) before redoing any of it.** Code + tests done (see 2026-09-09 session log). `cd sector2/apps/lifefirst/meds-worker` → `wrangler d1 execute lifefirst-db --file=schema.sql --remote` → `wrangler secret put` PHOENIX_AUTH (match the fleet) + PUSHOVER_TOKEN / PUSHOVER_USER_LAURIE / PUSHOVER_USER_JERRY / RESEND_API_KEY / LAURIE_EMAIL / CAREGIVER_EMAIL (+ optional Twilio quartet) → `wrangler deploy` → `POST /configure {user_id:2,...}` to switch Laurie's guardrail on → live end-to-end test per README. Then add `meds-worker` as a 4th leg in `rotate-phoenix-auth.sh`. Needs from Jerry: Cloudflare auth, a Pushover app + Laurie's device key + the app on her phone, Laurie's Gmail, his travel contact.
