...
## NEXT SESSION
- **Office — live checks (no rush):** (1) deploy `office-notify-worker` per `sector2/apps/office/notify-worker/README.md` — `wrangler d1 execute schema.sql`, set `PHOENIX_AUTH` + `RESEND_API_KEY` secrets, `wrangler deploy`, live tamper→SMS→ack test; add it to `rotate-phoenix-auth.sh` (3rd `PHOENIX_AUTH` consumer). (2) Launch the dashboard **OFFICE** button once — exercise the live window render / contextBridge / copilot. (3) Optional: a Google OAuth client id for the google sign-in path.
...
- **Office — pending live checks** (Jerry, no rush): (1) deploy `office-notify-worker` — `wrangler d1 execute schema.sql`, set `PHOENIX_AUTH` + `RESEND_API_KEY` secrets, `wrangler deploy`, one real tamper→SMS→ack test (runbook: `notify-worker/README.md`; "Cloudflare Email Workers" from DESIGN.md isn't a free arbitrary-recipient path — Resend is the transport). (2) Launch the **OFFICE** button in the dashboard once to exercise the live window render / contextBridge / copilot. (3) Optional: a Google OAuth client id to light up the google sign-in path (fingerprint + Windows cover the internal case).
