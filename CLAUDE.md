...
- **Office Phase 2, Modules 3-6** — build plan written 2026-09-07: `sector2/apps/office/PLAN-modules-3-6.md`. All four required, no demo/stub (Jerry: "it needs everything working"). Order: M3 (notify transport — new `office-notify-worker`, Cloudflare Email Workers, escalation loop, wire into `verifyIntegrity`) → M5 (identity resolver, `lib/identity.js`) → M6 (dual-pane UI, ScriptForge-launcher pattern) ∥ M4 (LibreOffice/Frank/Helix worker — 4a infra now, 4c waits on Helix-on-Debian). **Recommended first session: Module 3 end to end.** Office is owned by Phoenix (GPL v3), the PBM companies proof-the-concept via real use. PBM Consulting is a plan only right now (Laurie owns the structure for federal woman-owned/HUBZone/disadvantaged/disabled set-asides — see [[pbm-companies]]); "don't hurry it." Open Qs in the plan: author-side box for M4 (Windows vs Debian VM), binding-contract gate timing, and whether a federal-records target changes M3/M5 schema/crypto before they freeze.
