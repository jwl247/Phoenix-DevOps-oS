...
- [ ] Office (dual browser pane) — Phase 1 (core document engine) + Modules 1-2 of Phase 2 (embedded file format, D1 schema) done, see `sector2/apps/office/DESIGN.md`. Modules 3-6 (notification transport, LibreOffice/Frank wiring, Google/Windows sign-in, dual-pane UI) not started.
- [ ] Sketchpad/Concepts
...
## NEXT SESSION
- **Deploy Office Module 3** — `sector2/apps/office/notify-worker/README.md` has the runbook: apply `schema.sql` to `phoenix_dev_db`, set `PHOENIX_AUTH` + `RESEND_API_KEY` secrets, `wrangler deploy`, then the live tamper→SMS→ack round-trip test. Consider adding `office-notify-worker` to `rotate-phoenix-auth.sh` (it's a third `PHOENIX_AUTH` consumer now).
- **Finish unifying the installer** — `start.ps1` self-elevation + autostart `-RunLevel Highest` done 2026-09-06. Still to fold into one `install.ps1`: Helix autostart (`tools/poc/install-helix-autostart.ps1`), dashboard autostart registration, PowerShell-profile hook. Also add a target-path param to `install.ps1` so its hardcoded `$HOME\Phoenix` (C:) default stops fighting the real D: repo home.
...
- **Office Modules 4-6** — plan: `sector2/apps/office/PLAN-modules-3-6.md`. All required, no demo/stub (Jerry: "it needs everything working. consulting will proof the concept — not hurry it or sway the path"). **M3 (notification transport) is code-complete 2026-09-07** — `office-notify-worker` + `lib/tamper-guard.js` + `office_notifications` schema, 36/36 tests, dry-run clean; **pending Jerry's live deploy** (secrets + a Resend API key — see `notify-worker/README.md`; DESIGN.md's "Cloudflare Email Workers" turned out not to be a real free arbitrary-recipient path, Resend is the working transport). Next: **M5** (`lib/identity.js` — fingerprint/Windows-SID/Google-OAuth resolver, `office_authors` linking) → **M6** (dual-pane UI, ScriptForge-launcher pattern) ∥ **M4** (LibreOffice/Frank/Helix worker; 4a = deploy dormant `phoenix-unoserver.service`; 4c waits on Helix-on-Debian). Office is owned by Phoenix (GPL v3); the PBM companies proof-the-concept via real use — PBM Consulting is a plan only, Laurie owns the structure for federal woman-owned/HUBZone/disadvantaged/disabled set-asides (see [[pbm-companies]]). Open Qs in the plan: binding-contract gate timing, whether a federal-records target changes M5's schema/crypto before it freezes.
