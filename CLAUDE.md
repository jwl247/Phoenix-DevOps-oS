...
- **Security & Integrity Audit backlog** — full findings in `docs/sec audit doc from you to you.txt` (Claude-to-Claude, live adversarial code audit, Sept 2026, every finding traced to file/line/commit). Standing rule: show the actual call site / test output before marking any item done. Sequencing per the doc:
  - **T1 #4 first** — resurrect the guardian/honeypot system (`SECTOR4/copes/src/security/` — real design, zero call sites): move `security/` into the live tree, call `copes_runtime.boot()` from the real boot entrypoint, wire event sources (`phoenix_auth.py` failed login → `GuardianAlpha.handle_contact`, `usys.ps1` unrecognized/permission-violating suite → Beta event).
  - **T1 #1 + #3 together** — one "check before execution" mechanism: enforce `.suite.json` `permissions` in `usys.ps1` before `Invoke-UsysRun` (restrict FS writes to the suite's clonepool dir unless declared; refuse undeclared network) + require a local `PHOENIX_AUTH`-equivalent stamp on a suite before `usys run` executes it (unstamped → explicit `--i-know-this-is-unverified` flag, not silent trust).
  - **T2 #7** — declare a canonical intake path (`phoenix-core/tools/intake.py` vs `sector2/package-handler/intake.sh`) before touching either again; deprecate the other loudly. (Pairs with the sidecar JSON hardening item below.)
  ...
  - **T1 #2** — sandbox non-VM suites (Windows Job Object / capped token) once #1's permission model exists to scope against.
  - **T3 (parallel)** — `Ball.authorize()` (`franken5.py:119`) wired or docstring stripped; `Ball.for_family()` least-privilege-by-declaration not hardcoded constant; standing integrity job (generalize #5) — `hex_id` vs fresh content hash, cron'd (regressed once already: `repair_clonepool.py` fixed 136/137 rows).
