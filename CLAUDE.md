...
- **`usys intake` (bare top-level command) is STILL broken — separate pipeline from today's fixes above, not yet touched.** Confirmed live 2026-09-21: routes to `sector4/intake/intake.sh` (Sector 4's own vault intake — a deliberately separate, legitimate pipeline from `sector2/package-handler/intake.sh`, not a duplicate to merge). Two real, live bugs: (1) `cmd_dir`'s `for f in "${target}"/**/*(.); do` (line 59) is zsh glob syntax, invalid in bash — `usys intake <dir>` is broken. (2) Both `cmd_file` and `cmd_dir` call `phoenix-core/tools/intake.py` (`INTAKE_PY`) for every file — the same deprecated stub whose D1-sync now silently reports fake success under Cloudflare Access (found and fixed for the *other* 4 call sites today, see above — this one wasn't in scope of that pass since it's Sector 4's pipeline, not Sector 2's). `usys clone` (sector2/package-handler) remains the working, canonical path for dogfooding meanwhile.
