...
# ============================================================
# FIXES in 1.7.0:
#   - Removed PHOENIX_AUTH reassignment (was shadowing env var)
#   - Changed auth header to X-Phoenix-Auth (matches worker)
#   - Accept HTTP 201 as success (not just 200)
#   - Added -y / --yes flag for non-interactive dup handling
# ============================================================
# PIPELINE IN:
...
# PHOENIX_AUTH is read directly from environment — not reassigned here
...
intake_file() {
  ...
  if [[ "${dup_result}" == dup:* ]]; then
    ...
    if [[ "${YES_MODE}" == "true" ]]; then
      # Non-interactive: default to version it anyway
      log "INFO" "dup: -y mode — versioning anyway"
    else
      echo ""
...
╚═╝╚═╝  ╚═══╝  ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝
  ...
  FLAGS:
    -y, --yes                        Non-interactive mode (auto-version dups)
  ...
  Duplicate handling:
    ...
    Use -y to skip prompt and auto-version
...
intake_directory() {
  ...
  if [[ "${YES_MODE}" == "true" ]]; then
    log "INFO" "dir intake: -y mode — proceeding with full intake"
  else
