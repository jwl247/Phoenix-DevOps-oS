...
# ── Predictive engine — lives alongside this file in CoPES/src/kernel/
...
class Helix:
    ...
    def __init__(
        ...
        # Predictive prefetch — watches both strands, one window
        ...
        def heartbeat():
            while self._alive:
                ...
                self.log.info(
                    ...
                    f"predictive_obs={self.predictive.stats['observations']}"
                )
    ...
    def _handle_packet(self, raw: bytes):
        ...
        Predictive engine observes every key — learns, prefetches ahead.
        """
        try:
        ...
        # Predictive engine observes both strands — one shared window
        self.predictive.observe(key, pkt.payload_a, strand="A")
        self.predictive.observe(key, pkt.payload_b, strand="B")

        # Pull prefetch queue — warm predicted keys in both engines now
        for prefetch_key in self.predictive.get_prefetch_queue():
            ...
            cached = self.engine_b._cache.get(prefetch_key)
            if cached is not None:
                self.engine_a._cache[prefetch_key] = cached  # warm A from B
    ...
    def shutdown(self):
        ...
        # Predictive engine shuts down cleanly
...
def _run_demo():
    """Demo: Helix standalone — twin engines, predictive, replication, shared memory."""
    print("""
╔══════════════════════════════════════════════╗
║  CoPES — Double Helix Engine                  ║
║  Helix — Subprocess. Queen. Full Mobility.    ║
║  Predictive prefetch engine: online           ║
...
╚══════════════════════════════════════════════╝
    ...
    print("[ DEMO ] Twin engines + predictive engine online...")
    print(f"  Status: {helix.status()}")
    print()

    # Process packets — predictive engine observes
    print("[ DEMO ] Processing packets (predictive engine learning)...")
    test_data = [
        ...
        (2, b"Lane B payload - quadralingual packet data B"),  # cache hit
        (3, b"Lane C payload"),
        (1, b"Lane A payload - quadralingual packet data A"),  # hot key
    ]
    for key, data in test_data:
        result_a = helix.engine_a.process(key, data)
        result_b = helix.engine_b.process(key, data)
        helix.predictive.observe(key, data, strand="A")
        helix.predictive.observe(key, data, strand="B")
        print(f"  key={key} | A={len(result_a)}b | B={len(result_b)}b | "
              f"hit_rate={helix.engine_a.cache_hit_rate:.0%}")
    print()

    print("[ PREDICTIVE ] Status after learning:")
    ps = helix.predictive.status()
    for k, v in ps.items():
        print(f"  {k}: {v}")
    print(f"  hot_keys: {helix.predictive.hot_keys()[:3]}")
    print()

    # Large payload → shared memory
    print("[ DEMO ] Large payload → shared memory...")
    large_data = b"X" * 8192
    ...
    helix.set_window(8)
    print(f"[ DEMO ] Frank expanded window → {helix.max_replicas}")
    print()

    r4 = helix.replicate(ReplicaTarget.HELIX)
    if r4 and r4.proc:
        print(f"[ DEMO ] Helix replica PID={r4.proc.pid} ✓")
    else:
        print(f"[ DEMO ] Helix replica registered ✓")
    print()
